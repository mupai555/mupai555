...
def calcular_macros_alternativos(peso, grasa_corregida, mlg, tmb, sexo, nivel_entrenamiento, geaf, eta, gee_prom_dia):
    ...
    Args:
        peso: Peso corporal en kg
        grasa_corregida: % de grasa corporal corregida
        mlg: Masa libre de grasa en kg
        tmb: Tasa metabólica basal en kcal
        sexo: "Hombre" o "Mujer"
        nivel_entrenamiento: "principiante", "intermedio", "avanzado", "élite"
        geaf: Factor de actividad física
        eta: Factor de efecto térmico de alimentos
        gee_prom_dia: Gasto energético por ejercicio promedio diario en kcal
    
    Returns:
        dict: Diccionario con clasificación, fase, energía y macros calculados
    """
    try:
        # Sanitize inputs
        peso = float(peso)
...
if USER_VIEW:
    ...
    # ============== CÁLCULO DE MACROS ALTERNATIVOS (MUPAI AVANZADO) ==============
    # Esta lógica avanzada NO se muestra al usuario (solo en resumen administrativo/email)
    # Calcula macros usando el esquema MUPAI auditado de Fases, Energía y Macros
...
# ==================== EMAIL: CÁLCULO MUPAI AVANZADO (MACROS ALTERNATIVOS) ====================
# Esta sección es EXCLUSIVA para el resumen administrativo/email
# NO se muestra al usuario en la interfaz
# Incluye lógica avanzada MUPAI de clasificación, fases y macros
...
=====================================
🔬 ANÁLISIS MUPAI AVANZADO (SISTEMA AUDITADO)
=====================================
NOTA: Este análisis utiliza el sistema MUPAI auditado de Fases, Energía y Macros.
Es una metodología avanzada complementaria para análisis administrativo interno.

📊 CLASIFICACIÓN CORPORAL:
- Categoría por % grasa: {alt['clasificacion']}
- % Grasa corregida (DEXA): {grasa_corregida:.1f}%
- Interpretación: Basada en rangos científicos específicos por sexo

🎯 FASE NUTRICIONAL DETERMINADA:
- Fase recomendada: {alt['fase']}
- % Energía vs mantenimiento: {alt['porcentaje_energia']:+.1f}%
- Justificación: Fase automática basada en clasificación corporal y objetivos

⚡ CÁLCULO ENERGÉTICO:
- TDEE Mantenimiento: {alt['tdee_mantenimiento']:.0f} kcal/día
  (TMB {tmb:.0f} × GEAF {geaf if 'geaf' in locals() else 1.0} × ETA {eta if 'eta' in locals() else 1.1} + GEE {gee_prom_dia if 'gee_prom_dia' in locals() else 0:.0f})
- Calorías objetivo: {alt['calorias_objetivo']:.0f} kcal/día
- Ajuste aplicado: {alt['porcentaje_energia']:+.1f}% sobre mantenimiento

🥗 DISTRIBUCIÓN DE MACRONUTRIENTES:
- Proteína: {alt['proteina_g']:.1f}g ({alt['proteina_kcal']:.0f} kcal) = {alt['proteina_kcal']/alt['calorias_objetivo']*100 if alt['calorias_objetivo'] > 0 else 0:.1f}%
- Grasas: {alt['grasa_g']:.1f}g ({alt['grasa_kcal']:.0f} kcal) = {alt['grasa_kcal']/alt['calorias_objetivo']*100 if alt['calorias_objetivo'] > 0 else 0:.1f}%
- Carbohidratos: {alt['carbohidratos_g']:.1f}g ({alt['carbohidratos_kcal']:.0f} kcal) = {alt['carbohidratos_kcal']/alt['calorias_objetivo']*100 if alt['calorias_objetivo'] > 0 else 0:.1f}%
- Total: {alt['calorias_objetivo']:.0f} kcal/día

🔍 METODOLOGÍA APLICADA:
- Sistema: MUPAI — Lógica auditada de Fases, Energía y Macros
- Criterios: Clasificación por % grasa + Nivel entrenamiento + TDEE preciso
- Ventajas: Personalización avanzada basada en composición corporal real
- Aplicación: {'PSMF considerado' if alt.get('psmf_aplica', False) else 'Distribución tradicional optimizada'}

📝 COMPARACIÓN CON PLAN TRADICIONAL:
- Diferencia calórica: {alt['calorias_objetivo'] - plan_tradicional_calorias:+.0f} kcal/día
- Diferencia proteína: {alt['proteina_g'] - proteina_g_tradicional:+.1f}g/día
- Enfoque: {'Más agresivo' if alt['calorias_objetivo'] < plan_tradicional_calorias else 'Más conservador' if alt['calorias_objetivo'] > plan_tradicional_calorias else 'Similar'}

ℹ️ NOTA IMPORTANTE:
Este cálculo alternativo sirve como referencia adicional para el análisis administrativo.
El plan presentado al cliente sigue siendo el tradicional o PSMF según corresponda.
Este análisis permite validación cruzada y refinamiento continuo de protocolos.
"""
